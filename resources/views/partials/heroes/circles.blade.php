...
<div class="relative min-h-0 flex items-center justify-center overflow-hidden bg-[#050505] py-10 sm:py-14 md:py-16">
    ...
    <div class="absolute inset-0 max-w-7xl mx-auto pointer-events-none z-20">
        ...
        @foreach($circlePositions as $pos)
            <div class="absolute flex flex-col items-center group pointer-events-auto cursor-pointer circle-float opacity-40 sm:opacity-75 md:opacity-100" style="top: {{ $pos['top'] }}; left: {{ $pos['left'] ?? 'auto' }}; right: {{ $pos['right'] ?? 'auto' }}; animation-delay: {{ $pos['delay'] }};">
                <div class="w-8 h-8 sm:w-10 sm:h-10 md:w-14 md:h-14 rounded-full bg-gradient-to-br from-[#1c0808] via-[#111] to-[#0a0a0a] border border-[#FF2121]/40 shadow-lg md:shadow-xl shadow-[#FF2121]/20 flex items-center justify-center text-white text-xs sm:text-base md:text-xl group-hover:scale-115 group-hover:border-[#FF2121] group-hover:shadow-[0_0_25px_rgba(255,33,33,0.5)] transition-all duration-300 relative">
                    ...
                    <i class="{{ $pos['icon'] }} group-hover:text-[#FF2121] transition-colors"></i>
                </div>
    ...
    <div class="relative z-10 w-full max-w-4xl mx-auto px-4 sm:px-6 flex flex-col items-center text-center">
        ...
        <p class="text-sm sm:text-base md:text-lg text-gray-400 max-w-xl mb-6 sm:mb-8 leading-relaxed font-medium px-2">
            {{ $settings['hero_description'] ?? 'Descarga directa con actualizaciones automáticas. Formateado para agencias, desarrolladores y creadores de contenido.' }}
        </p>
        ...
        <div class="flex flex-col sm:flex-row justify-center items-stretch sm:items-center gap-3 sm:gap-4 w-full sm:w-auto px-4 sm:px-0">
            @auth
            <a href="{{ route('products.index') }}" class="w-full sm:w-auto px-8 py-3.5 bg-gradient-to-r from-[#FF2121] to-[#F51B1B] text-white font-black text-xs uppercase tracking-wider rounded-2xl hover:scale-105 transition-all shadow-2xl shadow-[#FF2121]/40 flex items-center justify-center gap-2.5">
                <i class="fas fa-rocket text-sm"></i> Explora el Catálogo
            </a>
            @else
            <a href="{{ route('register') }}" class="w-full sm:w-auto px-8 py-3.5 bg-gradient-to-r from-[#FF2121] to-[#F51B1B] text-white font-black text-xs uppercase tracking-wider rounded-2xl hover:scale-105 transition-all shadow-2xl shadow-[#FF2121]/40 flex items-center justify-center gap-2.5">
