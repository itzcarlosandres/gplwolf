<form action="{{ route('admin.settings.sidebar.update') }}" method="POST">
    @csrf
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <label class="cursor-pointer">
                            <input type="radio" name="sidebar_type" value="{{ $key }}" class="peer sr-only" {{ ($settings['sidebar_type'] ?? '') == $key ? 'checked' : '' }}>
                            <div class="bg-[#080808] border border-white/[0.08] rounded-xl p-4 flex items-center gap-3 peer-checked:border-[#FF2121]/50 peer-checked:bg-[#FF2121]/10 transition-all hover:border-white/20">
                                ...
                                <div>
                                    <span class="text-sm font-bold text-white block">{{ $type['name'] }}</span>
                                    <span class="text-[10px] text-gray-500 uppercase tracking-wider font-medium">{{ $type['desc'] }}</span>
                                </div>
                            </div>
                        </label>
                        @endforeach
