<form action="{{ route('admin.settings.update-general') }}" method="POST" enctype="multipart/form-data" class="space-y-6"
      x-data="{ 
          identityType: '{{ $settings['site_identity_type'] ?? 'logo' }}', 
          selectedIcon: '{{ $settings['site_icon'] ?? 'fas fa-store' }}' 
      }">
    @csrf
    ...
    <div class="bg-[#0a0a0a] border border-white/[0.06] rounded-2xl overflow-hidden">
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="site_name" value="{{ $settings['site_name'] ?? 'WP Marketplace' }}"
            ...
            <div x-show="identityType === 'text'" x-transition>
                ...
                <div class="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-12 gap-2">
                    ...
                    <label class="cursor-pointer group">
                        <input type="radio" name="site_icon" value="{{ $icon }}" class="peer sr-only" x-model="selectedIcon">
                        <div class="aspect-square flex items-center justify-center rounded-xl bg-[#080808] border border-white/[0.08] peer-checked:bg-[#F51B1B] peer-checked:border-[#F51B1B] peer-checked:text-white hover:border-white/20 transition-all">
                            <i class="{{ $icon }} text-lg" :class="selectedIcon === '{{ $icon }}' ? 'text-white' : 'text-gray-400 group-hover:text-gray-200'"></i>
                        </div>
                    </label>
                    @endforeach
                </div>
                    ...
                    <div class="relative group cursor-pointer">
                        <div class="border border-dashed border-white/[0.08] rounded-xl p-6 text-center transition-all hover:border-[#F51B1B]/50 hover:bg-white/[0.02] bg-[#080808]">
                            ...
                            @if(isset($settings['site_logo']))
                                <img src="{{ asset($settings['site_logo']) }}" class="h-16 mx-auto mb-4 object-contain">
                    ...
                    <div class="relative group cursor-pointer">
                        <div class="border border-dashed border-white/[0.08] rounded-xl p-6 text-center transition-all hover:border-[#F51B1B]/50 hover:bg-white/[0.02] bg-[#080808]">
                            ...
                            @if(isset($settings['site_favicon']))
                                <img src="{{ asset($settings['site_favicon']) }}" class="h-12 w-12 mx-auto mb-4 object-contain">
    ...
    <!-- Typography Section -->
    <div class="bg-[#0a0a0a] border border-white/[0.06] rounded-2xl overflow-hidden"
         x-data="{ selectedFont: '{{ $settings['site_font'] ?? 'Outfit' }}' }">
        <style>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                @foreach($fonts as $font => $desc)
                <label class="cursor-pointer group">
                    <input type="radio" name="site_font" value="{{ $font }}" class="peer sr-only" x-model="selectedFont">
                    <div class="bg-[#080808] border border-white/[0.08] rounded-xl p-4 peer-checked:border-pink-500/50 peer-checked:bg-pink-500/10 hover:border-white/20 transition-all h-full flex flex-col gap-2">
                        <div class="flex justify-between items-center">
                            <span class="text-xs font-black text-gray-400 uppercase tracking-wider">{{ $font }}</span>
                        ...
                        <div style="font-family: '{{ $font }}', sans-serif;">
                            ...
                            <span class="text-xs text-gray-500 font-medium">{{ $desc }}</span>
                        </div>
                    </div>
                </label>
                @endforeach
        ...
        <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="home_meta_title" value="{{ $settings['home_meta_title'] ?? ($settings['site_name'] ?? 'WP Marketplace') }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="home_meta_description" rows="3"
                        class="w-full bg-[#080808] border border-white/[0.08] rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#FF2121]/50 focus:ring-2 focus:ring-[#FF2121]/10 transition-all font-bold placeholder-gray-700">{{ $settings['home_meta_description'] ?? 'Descarga los mejores temas y plugins premium para WordPress.' }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="home_meta_keywords" value="{{ $settings['home_meta_keywords'] ?? 'wordpress, themes, plugins, premium, marketplace' }}"
                        class="w-full bg-[#080808] border border-white/[0.08] rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#FF2121]/50 focus:ring-2 focus:ring-[#FF2121]/10 transition-all font-bold placeholder-gray-700">
            ...
            <div class="pt-6 border-t border-white/[0.06]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="product_seo_title_template" value="{{ $settings['product_seo_title_template'] ?? 'Descargar %name% - Premium GPL' }}"
                            class="w-full bg-[#080808] border border-white/[0.08] rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#FF2121]/50 focus:ring-2 focus:ring-[#FF2121]/10 transition-all font-bold placeholder-gray-700">
                    ...
                    <div>
                        ...
                        <input type="text" name="product_seo_desc_template" value="{{ $settings['product_seo_desc_template'] ?? 'Descargar %name% - %description%' }}"
                            class="w-full bg-[#080808] border border-white/[0.08] rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#FF2121]/50 focus:ring-2 focus:ring-[#FF2121]/10 transition-all font-bold placeholder-gray-700">
            ...
            <div>
                ...
                <div class="relative group cursor-pointer">
                    <div class="border border-dashed border-white/[0.08] rounded-xl p-6 text-center transition-all hover:border-[#FF2121]/50 hover:bg-white/[0.02] bg-[#080808] flex flex-col items-center justify-center">
                        ...
                        @if(isset($settings['site_og_image']))
                            <div class="relative w-full max-w-sm mb-4 rounded-lg overflow-hidden border border-white/[0.06]">
                                <img src="{{ asset($settings['site_og_image']) }}" class="w-full h-auto object-cover">
            ...
            <div class="pt-6 border-t border-white/[0.06]">
                ...
                <div class="bg-[#080808] rounded-xl overflow-hidden border border-white/[0.08]">
                    ...
                    <textarea name="site_header_code" rows="6"
                        ...
                        placeholder="<!-- Google Analytics, Verification Tags, etc. -->">{{ $settings['site_header_code'] ?? '' }}</textarea>
                </div>
...
<form id="remove-logo-form" action="{{ route('admin.settings.remove-image') }}" method="POST" class="hidden">
    @csrf
    <input type="hidden" name="key" value="site_logo">
</form>

<form id="remove-favicon-form" action="{{ route('admin.settings.remove-image') }}" method="POST" class="hidden">
    @csrf
    <input type="hidden" name="key" value="site_favicon">
</form>

<form id="remove-og-form" action="{{ route('admin.settings.remove-image') }}" method="POST" class="hidden">
    @csrf
