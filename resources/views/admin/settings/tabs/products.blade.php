<form action="{{ route('admin.settings.products.update') }}" method="POST" class="space-y-6">
    @csrf
    ...
    <div class="bg-[#0a0a0a] border border-white/[0.06] rounded-2xl overflow-hidden">
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
                ...
                <input type="text" name="products_section_title" value="{{ $settings['products_section_title'] ?? 'Lo más Vendido' }}" required
            ...
            <div>
                ...
                <div class="grid grid-cols-4 gap-2">
                    @foreach([3,4,5,6] as $cols)
                    <label class="cursor-pointer">
                        <input type="radio" name="products_grid_columns" value="{{ $cols }}" class="peer sr-only" {{ ($settings['products_grid_columns'] ?? 6) == $cols ? 'checked' : '' }}>
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" name="products_per_page" value="{{ $settings['products_per_page'] ?? 24 }}" min="6" max="100" step="6" required
                        class="w-full bg-[#080808] border border-white/[0.08] rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#FF2121]/50 focus:ring-2 focus:ring-[#FF2121]/10 transition-all font-bold placeholder-gray-700">
        ...
        <div class="px-6 pb-6">
            ...
            <div class="bg-[#080808] border border-white/[0.06] rounded-xl p-6">
                <div class="flex justify-center items-end gap-2 h-24">
                    @for($i=0; $i<6; $i++)
                        <div class="w-12 bg-gradient-to-t from-[#FF2121]/30 to-[#F51B1B]/5 rounded-t-md border border-[#FF2121]/20 {{ $i >= (($settings['products_grid_columns'] ?? 6)) ? 'opacity-30 hidden md:block' : '' }}" style="height: {{ 40 + ($i % 3) * 25 }}px"></div>
                    @endfor
                </div>
                <p class="text-center text-[10px] text-gray-600 mt-4 uppercase tracking-wider font-bold">{{ $settings['products_grid_columns'] ?? 6 }} columnas visibles</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-[#0a0a0a] border border-white/[0.06] rounded-2xl overflow-hidden">
        ...
        <div class="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div>
                ...
                <input type="text" name="home_featured_title" value="{{ $settings['home_featured_title'] ?? 'Lo más Vendido' }}" required
            ...
            <div class="lg:col-span-2">
                ...
                <input type="text" name="home_featured_description" value="{{ $settings['home_featured_description'] ?? 'Explora nuestras últimas novedades premium para WordPress.' }}" required
            ...
            <div class="lg:col-span-3">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
                    ...
                    <label class="cursor-pointer">
                        <input type="radio" name="home_products_style" value="{{ $key }}" class="peer sr-only" {{ ($settings['home_products_style'] ?? 'grid') == $key ? 'checked' : '' }}>
                        <div class="relative p-5 bg-[#080808] border border-white/[0.08] rounded-xl peer-checked:border-[#FF2121]/50 peer-checked:bg-[#FF2121]/10 transition-all hover:border-white/20 h-full">
                            ...
                            <h4 class="text-white font-bold text-sm">{{ $style['name'] }}</h4>
                            <p class="text-[10px] text-gray-500 mt-1 font-medium uppercase tracking-wider">{{ $style['desc'] }}</p>
                        </div>
                    </label>
                    @endforeach
                </div>
            ...
            <div>
                ...
                <div class="relative">
                    <select name="home_grid_columns" class="w-full bg-[#080808] border border-white/[0.08] rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#FF2121]/50 focus:ring-2 focus:ring-[#FF2121]/10 transition-all font-bold appearance-none">
                        <option value="3" {{ ($settings['home_grid_columns'] ?? 4) == 3 ? 'selected' : '' }}>3 Columnas</option>
                        <option value="4" {{ ($settings['home_grid_columns'] ?? 4) == 4 ? 'selected' : '' }}>4 Columnas</option>
                        <option value="5" {{ ($settings['home_grid_columns'] ?? 4) == 5 ? 'selected' : '' }}>5 Columnas</option>
                        <option value="6" {{ ($settings['home_grid_columns'] ?? 4) == 6 ? 'selected' : '' }}>6 Columnas</option>
                    </select>
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" name="home_products_count" value="{{ $settings['home_products_count'] ?? 6 }}" min="1" max="100" required
                        class="w-full bg-[#080808] border border-white/[0.08] rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#FF2121]/50 focus:ring-2 focus:ring-[#FF2121]/10 transition-all font-bold placeholder-gray-700">
                    <span class="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold text-gray-600 uppercase">prod</span>
                </div>
            </div>
