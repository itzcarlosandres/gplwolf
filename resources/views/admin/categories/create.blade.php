...
<div class="mb-8 flex items-center">
    ...
    <div>
        <h1 class="text-3xl font-bold text-white tracking-tight">Nueva Categoría</h1>
        <p class="text-gray-400 mt-1">Define una nueva etiqueta para tus productos.</p>
    </div>
</div>
...
<form action="{{ route('admin.categories.store') }}" method="POST" class="max-w-2xl">
    ...
    <div class="bg-gray-800/40 backdrop-blur-xl p-8 rounded-[32px] border border-white/5 shadow-2xl space-y-6">
        <div>
            ...
            <input type="text" name="name" required class="w-full bg-gray-950 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-[#FF2121]/50 transition-all" placeholder="Ej: E-commerce, Blog, Seguridad...">
        ...
        <div>
            ...
            <input type="text" name="icon" class="w-full bg-gray-950 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:ring-2 focus:ring-[#FF2121]/50 transition-all font-mono" placeholder="Ej: fas fa-rocket">
        </div>

        <div class="space-y-4">
            <div class="flex items-center p-5 bg-[#FF2121]/5 rounded-2xl border border-[#FF2121]/10">
                <input type="hidden" name="is_active" value="0">
                <input type="checkbox" name="is_active" value="1" id="is_active" checked class="w-6 h-6 text-[#FF2121] bg-gray-900 border-white/10 rounded-lg focus:ring-[#FF2121] transition-all">
            ...
            <div class="flex items-center p-5 bg-rose-500/5 rounded-2xl border border-rose-500/10">
                ...
                <input type="checkbox" name="exclude_from_membership" value="1" id="exclude_from_membership" class="w-6 h-6 text-rose-500 bg-gray-900 border-white/10 rounded-lg focus:ring-rose-500 transition-all">
        ...
        <button type="submit" class="w-full gradient-bg text-white py-5 rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl shadow-[#F51B1B]/30 hover:opacity-90 transition-all transform active:scale-95 leading-none">
            Crear Categoría <i class="fas fa-check-circle ml-2"></i>
        </button>
    </div>
</form>
@endsection