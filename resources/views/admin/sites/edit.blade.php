...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-white tracking-tight">Editar Dominio Conectado</h1>
            <p class="text-gray-400 text-sm mt-1">Modifica los detalles del sitio o reasigna el cliente asociado.</p>
        </div>
    ...
    <div class="bg-[#0c0c0c] border border-white/5 rounded-3xl overflow-hidden shadow-2xl">
        <form action="{{ route('admin.sites.update', $site->id) }}" method="POST" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-2">
                ...
                <select name="user_id" id="user_id" class="block w-full rounded-xl border-0 bg-gray-900 py-3 text-white shadow-sm ring-1 ring-inset ring-white/10 focus:ring-2 focus:ring-inset focus:ring-[#FF2121] sm:text-sm">
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ old('user_id', $site->user_id) == $user->id ? 'selected' : '' }}>
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="domain" id="domain" value="{{ old('domain', $site->domain) }}" placeholder="https://tusitioweb.com" class="block w-full rounded-xl border-0 bg-gray-900 py-3 text-white shadow-sm ring-1 ring-inset ring-white/10 focus:ring-2 focus:ring-inset focus:ring-[#FF2121] sm:text-sm" required>
            ...
            <div class="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.04] flex gap-4">
                ...
                <div class="text-xs leading-relaxed text-gray-400">
                    <strong class="text-white block mb-1">Impacto de Modificación</strong>
                    Si cambias el dominio del sitio conectado, el token anterior que usaba el plugin de WordPress en el dominio antiguo será revocado de inmediato por razones de seguridad. El cliente deberá iniciar sesión de nuevo en su panel de WordPress para reconectar el sitio al nuevo dominio configurado.
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-white/5">
                ...
                <button type="submit" class="px-8 py-3 bg-blue-500 hover:bg-blue-600 text-white font-black text-sm uppercase tracking-wider rounded-xl shadow-lg shadow-blue-500/20 transition-all flex items-center gap-2">
