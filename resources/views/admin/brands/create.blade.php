...
@section('title', 'Nuevo Elemento / Anuncio')

@section('content')
<div class="max-w-3xl" x-data="{ isPromo: {{ old('is_promo', 0) ? 'true' : 'false' }} }">
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-black text-white tracking-tight">Nuevo Elemento en Carrusel</h1>
        <p class="text-gray-500 text-sm mt-1">Crea una marca de confianza o un anuncio promocional (como el Trial de 7 Días) para la Home.</p>
    </div>

    <form action="{{ route('admin.brands.store') }}" method="POST" class="bg-[#111111] border border-white/[0.06] rounded-2xl p-6 sm:p-8 space-y-6 shadow-2xl">
        @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="icon" value="{{ old('icon', 'fas fa-cube') }}" placeholder="fas fa-cube o fas fa-bolt" 
                       class="w-full bg-[#0d0d0d] border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-amber-500/50 transition-all">
        ...
        <div class="space-y-6 pt-2 border-t border-white/[0.04]" x-show="isPromo" x-transition>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="text" name="link_url" value="{{ old('link_url', '/membresias/prueba-gratis') }}" 
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="badge_text" value="{{ old('badge_text', '⚡ TRIAL 7 DÍAS') }}" 
                ...
                <div class="grid grid-cols-3 sm:grid-cols-5 gap-3">
                    @foreach(['amber' => 'Ámbar / Dorado', 'red' => 'Rojo Brillante', 'emerald' => 'Verde Éxito', 'blue' => 'Azul Pro', 'purple' => 'Púrpura VIP'] as $colorKey => $colorName)
                        <label class="flex items-center gap-2 p-2.5 rounded-xl border border-white/10 bg-[#0d0d0d] cursor-pointer hover:border-white/30 transition-all text-xs font-bold">
                            <input type="radio" name="highlight_color" value="{{ $colorKey }}" {{ old('highlight_color', 'amber') === $colorKey ? 'checked' : '' }} class="text-[#FF2121] focus:ring-0">
