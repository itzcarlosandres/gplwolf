        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h3 class="text-base font-bold text-white flex items-center gap-2">
                    Tienes {{ $pendingUpdatesList->count() }} solicitud{{ $pendingUpdatesList->count() > 1 ? 'es' : '' }} de actualización pendiente{{ $pendingUpdatesList->count() > 1 ? 's' : '' }}
                </h3>
                <p class="text-xs text-gray-400 mt-0.5">
                    Última por <strong class="text-white">{{ $pendingUpdatesList->first()->user->name ?? 'Usuario' }}</strong> para <strong class="text-amber-400">{{ $pendingUpdatesList->first()->product->name ?? 'Producto' }}</strong>
                </p>
            </div>
        </div>
        <a href="{{ route('admin.update-requests.index') }}" class="px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-black font-black text-xs uppercase tracking-wider rounded-xl transition-all shadow-lg shadow-amber-500/20 whitespace-nowrap">
            Atender Solicitudes →
        </a>
    </div>
@endif
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    <div class="bg-gray-800/40 backdrop-blur-xl p-6 rounded-2xl border border-white/5 flex items-center group hover:bg-gray-800/60 transition-all duration-300">
        ...
        <div class="ml-4">
            ...
            <p class="text-2xl font-bold text-white mt-1">{{ number_format($stats['total_products']) }}</p>
        </div>
    </div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
    ...
    <div class="bg-gray-800/40 backdrop-blur-xl rounded-2xl border border-white/5 overflow-hidden flex flex-col">
        <div class="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
            ...
            <a href="{{ route('admin.orders.index') }}" class="text-[#FF2121] hover:text-[#FF2121] text-sm font-semibold transition-colors">
                ...
                <tbody class="divide-y divide-white/5 text-sm">
                    @forelse($stats['recent_orders'] as $order)
                        <tr class="hover:bg-white/5 transition-colors group">
                            <td class="px-6 py-4 font-mono text-[#FF2121] group-hover:text-[#FF2121]">#{{ $order->order_number }}</td>
                            <td class="px-6 py-4 text-gray-300">{{ $order->user->name ?? 'Usuario' }}</td>
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2.5 py-1 rounded-lg text-xs font-bold uppercase tracking-wider
                                    {{ $order->status === 'completed' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border border-amber-500/20' }}">
                                    {{ $order->status }}
                                </span>
                            </td>
                        </tr>
    ...
    <div class="bg-gray-800/40 backdrop-blur-xl rounded-2xl border border-white/5 overflow-hidden flex flex-col">
        <div class="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
            ...
            <a href="{{ route('admin.products.index') }}" class="text-[#FF2121] hover:text-[#FF2121] text-sm font-semibold transition-colors">
                    ...
                    <div class="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 transition-all group border border-transparent hover:border-white/5">
                        <div class="flex items-center">
                            <div class="w-12 h-12 bg-gray-900 rounded-lg flex items-center justify-center text-xl mr-4 border border-white/10 group-hover:rotate-6 transition-transform">
                                @if($product->type === 'theme') 🎨 @else ⚡ @endif
                            </div>
                            <div>
                                <h4 class="font-bold text-white group-hover:text-[#FF2121] transition-colors">{{ $product->name }}</h4>
                                <div class="flex items-center gap-2 mt-0.5">
                                    <span class="text-[10px] uppercase font-bold text-gray-500 tracking-wider bg-gray-900 px-1.5 py-0.5 rounded border border-white/5">{{ $product->type }}</span>
                                    <span class="text-[10px] text-gray-400">• {{ $product->category->name ?? 'Sin Categoría' }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="text-right">
                            <p class="text-lg font-bold text-white">{{ $product->downloads_count }}</p>
