...
<head>
    ...
    <title>Recuperar Contraseña | CaletaWP</title>
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center p-6 relative">
        ...
        <div class="w-full max-w-md relative z-10">
            ...
            <div class="text-center mb-8">
                <h1 class="text-3xl font-black text-white mb-3">¿Olvidaste tu contraseña?</h1>
                <p class="text-gray-500 text-sm font-medium leading-relaxed">
                    No te preocupes. Introduce tu correo electrónico y te enviaremos un enlace de recuperación para restablecerla.
                </p>
            </div>

            <!-- Session Status (Exito al enviar enlace) -->
            @if (session('status'))
            ...
            <form method="POST" action="{{ route('password.email') }}" class="space-y-6">
                ...
                <button type="submit" 
                    class="w-full gradient-bg hover:opacity-95 text-white font-bold py-4 px-6 rounded-xl transition duration-300 shadow-lg shadow-[#FF2121]/20 shine text-sm uppercase tracking-wider">
                    Enviar Enlace de Recuperación
                </button>
            </form>

            <div class="mt-8 text-center">
                <a href="{{ route('login') }}" class="text-xs font-bold text-gray-500 hover:text-white transition flex items-center justify-center gap-2">
                    <i class="fas fa-arrow-left text-[10px]"></i> Volver a Iniciar Sesión
                </a>
            </div>
        </div>
    </div>
