...
<head>
    ...
    <!-- Dynamic Favicon -->
    <?php if(isset($globalSettings['site_favicon'])): ?>
        <link rel="icon" href="<?php echo e(\Illuminate\Support\Str::startsWith($globalSettings['site_favicon'], 'ui/') ? asset($globalSettings['site_favicon']) : asset('storage/' . $globalSettings['site_favicon'])); ?>">
    <?php endif; ?>

    <title><?php echo $__env->yieldContent('meta_title', $globalSettings['home_meta_title'] ?? ($globalSettings['site_name'] ?? 'WP Marketplace')); ?></title>
    <meta name="description" content="<?php echo $__env->yieldContent('meta_description', $globalSettings['home_meta_description'] ?? 'Descarga los mejores Themes y Plugins Premium para WordPress.'); ?>">
    <meta name="keywords" content="<?php echo $__env->yieldContent('meta_keywords', $globalSettings['home_meta_keywords'] ?? ''); ?>">
    <link rel="canonical" href="<?php echo $__env->yieldContent('canonical', url()->current()); ?>" />

    <!-- Open Graph / Facebook -->
    <meta property="og:type" content="website">
    <meta property="og:url" content="<?php echo e(url()->current()); ?>">
    <meta property="og:site_name" content="<?php echo e($globalSettings['site_name'] ?? 'WP Marketplace'); ?>">
    <meta property="og:title" content="<?php echo $__env->yieldContent('meta_title', $globalSettings['home_meta_title'] ?? ($globalSettings['site_name'] ?? 'WP Marketplace')); ?>">
    <meta property="og:description" content="<?php echo $__env->yieldContent('meta_description', $globalSettings['home_meta_description'] ?? 'Themes y Plugins Premium para WordPress.'); ?>">
    <meta property="og:image" content="<?php echo $__env->yieldContent('meta_image', isset($globalSettings['site_og_image']) ? (\Illuminate\Support\Str::startsWith($globalSettings['site_og_image'], 'ui/') ? asset($globalSettings['site_og_image']) : asset('storage/' . $globalSettings['site_og_image'])) : asset('images/og-default.jpg')); ?>">

    <!-- Twitter -->
    <meta property="twitter:card" content="summary_large_image">
    <meta property="twitter:url" content="<?php echo e(url()->current()); ?>">
    <meta property="twitter:title" content="<?php echo $__env->yieldContent('meta_title', $globalSettings['home_meta_title'] ?? ($globalSettings['site_name'] ?? 'WP Marketplace')); ?>">
    <meta property="twitter:description" content="<?php echo $__env->yieldContent('meta_description', $globalSettings['home_meta_description'] ?? 'Themes y Plugins Premium para WordPress.'); ?>">
    <meta property="twitter:image" content="<?php echo $__env->yieldContent('meta_image', isset($globalSettings['site_og_image']) ? (\Illuminate\Support\Str::startsWith($globalSettings['site_og_image'], 'ui/') ? asset($globalSettings['site_og_image']) : asset('storage/' . $globalSettings['site_og_image'])) : asset('images/og-default.jpg')); ?>">
    ...
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="<?php echo e('https://fonts.googleapis.com/css2?family=' . str_replace(' ', '+', $globalSettings['site_font'] ?? 'Outfit') . ':wght@300;400;500;600;700;800;900&display=swap'); ?>" rel="stylesheet" media="print" onload="this.media='all'">
    <noscript>
        <link href="<?php echo e('https://fonts.googleapis.com/css2?family=' . str_replace(' ', '+', $globalSettings['site_font'] ?? 'Outfit') . ':wght@300;400;500;600;700;800;900&display=swap'); ?>" rel="stylesheet">
    </noscript>
    ...
    <style>
        body {
            font-family: '<?php echo e($globalSettings['site_font'] ?? 'Outfit'); ?>', sans-serif;
    ...
    <!-- Custom Header Code -->
    <?php echo $globalSettings['site_header_code'] ?? ''; ?>
...
<body class="bg-[#050505] text-gray-300 antialiased overflow-x-hidden selection:bg-[#FF2121]/30 selection:text-white" x-data="{ mobileMenu: false, mobileSearch: false, topbarClosed: localStorage.getItem('topbar_closed') === 'true' }">
    ...
    <div x-show="!topbarClosed" 
         ...
         style="background: linear-gradient(135deg, <?php echo e($topbarBgColor); ?> 0%, <?php echo e($topbarBgColor); ?>dd 100%);">
        ...
        <div class="relative max-w-7xl mx-auto px-6 flex items-center justify-between gap-4">
            <div class="flex-1 flex items-center justify-center gap-3">
                <span class="text-sm font-bold tracking-wide"><?php echo e($topbarText); ?></span>
                <?php if($topbarLink && $topbarLinkText): ?>
                <a href="<?php echo e($topbarLink); ?>" class="px-4 py-1.5 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-lg text-xs font-black uppercase tracking-widest transition-all hover:scale-105">
                    <?php echo e($topbarLinkText); ?> →
                </a>
                <?php endif; ?>
            </div>
    ...
    <nav class="sticky top-0 z-50 bg-[#0a0a0a] border-b border-white/5">
        <div class="mx-4 md:mx-8 lg:mx-12 pt-4 pb-3">
            ...
            <div class="flex items-center justify-between gap-4 px-2">
                <!-- Logo -->
                <a href="<?php echo e(route('home')); ?>" class="flex items-center gap-2 text-white font-black text-lg tracking-tight shrink-0 group">
                    <?php if(($globalSettings['site_identity_type'] ?? 'logo') === 'logo' && isset($globalSettings['site_logo'])): ?>
                        <img src="<?php echo e(\Illuminate\Support\Str::startsWith($globalSettings['site_logo'], 'ui/') ? asset($globalSettings['site_logo']) : asset('storage/' . $globalSettings['site_logo'])); ?>" alt="<?php echo e($globalSettings['site_name'] ?? 'WP Marketplace'); ?>" class="h-8 w-auto hover:scale-105 transition-transform">
                    <?php else: ?>
                        <div class="w-8 h-8 gradient-bg rounded-lg flex items-center justify-center text-sm text-white shadow-lg shadow-[#F51B1B]/40 group-hover:scale-110 transition-transform">
                            <i class="<?php echo e($globalSettings['site_icon'] ?? 'fas fa-store'); ?>"></i>
                        </div>
                        <span class="hidden sm:block"><?php echo e($globalSettings['site_name'] ?? 'CaletaWP'); ?></span>
                    <?php endif; ?>
                </a>
                ...
                <div class="hidden md:block flex-1 max-w-md relative" x-data="{ searchOpen: false, searchQuery: '', searchResults: [], loading: false }" @click.away="searchOpen = false" @keydown.window.prevent.cmd.k="$refs.searchInput.focus()" @keydown.window.prevent.ctrl.k="$refs.searchInput.focus()">
                        ...
                        <input 
                            ...
                            @input.debounce.300ms="
                                if (searchQuery.length >= 2) {
                                    loading = true;
                                    fetch('<?php echo e(route('search.live')); ?>?q=' + encodeURIComponent(searchQuery))
                                        .then(r => r.json())
                                        .then(data => {
                                            searchResults = data;
                                            searchOpen = true;
                    ...
                    >
                        ...
                        <div class="border-t border-white/10 p-3 bg-white/5">
                            <a :href="'<?php echo e(route('search.index')); ?>?q=' + encodeURIComponent(searchQuery)" class="text-[#FF2121] text-sm font-bold hover:text-[#FF2121] transition-colors flex items-center justify-center gap-2">
                ...
                <div class="hidden lg:flex items-center gap-1.5">
                    <a href="<?php echo e(route('products.index')); ?>" class="group flex items-center px-3 py-2 text-xs font-bold text-gray-400 hover:text-white transition-all rounded-xl hover:bg-white/5 <?php echo e(request()->routeIs('products.index') ? 'text-white bg-white/5' : ''); ?>">
                        <i class="fas fa-cubes text-[11px] mr-1.5 text-gray-500 group-hover:text-gray-200 transition-colors"></i>
                        Productos
                    </a>
                    <a href="<?php echo e(route('updates.index')); ?>" class="group flex items-center px-3 py-2 text-xs font-bold text-gray-400 hover:text-white transition-all rounded-xl hover:bg-white/5 <?php echo e(request()->routeIs('updates.index') ? 'text-white bg-white/5' : ''); ?>">
                        <i class="fas fa-bolt text-[11px] mr-1.5 text-gray-500 group-hover:text-gray-200 transition-colors"></i>
                        Novedades
                    </a>
                    <a href="<?php echo e(route('membership.pricing')); ?>" class="group flex items-center px-3 py-2 text-xs font-bold text-gray-400 hover:text-white transition-all rounded-xl hover:bg-white/5 <?php echo e(request()->routeIs('membership.pricing') ? 'text-white bg-white/5' : ''); ?>">
                        <i class="fas fa-crown text-[11px] mr-1.5 text-gray-500 group-hover:text-gray-200 transition-colors"></i>
                        Planes
                    </a>
                    <a href="<?php echo e(route('user.support.index')); ?>" class="group flex items-center px-3 py-2 text-xs font-bold text-gray-400 hover:text-white transition-all rounded-xl hover:bg-white/5 <?php echo e(request()->routeIs('user.support.index') ? 'text-white bg-white/5' : ''); ?>">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    ...
                    <?php if(auth()->guard()->check()): ?>
                        <a href="<?php echo e(route('dashboard')); ?>" class="hidden md:flex items-center gap-2 bg-white/5 border border-white/10 pl-1.5 pr-3 py-1.5 rounded-xl hover:bg-white/10 transition-all group">
                            <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode(Auth::user()->name)); ?>&background=6366f1&color=fff&bold=true" class="w-6 h-6 rounded-lg shadow-lg">
                            <span class="text-[10px] font-black uppercase tracking-widest text-gray-300 group-hover:text-white">Cuenta</span>
                        </a>
                    <?php else: ?>
                        <!-- Login Soft Pop -->
                        <a href="<?php echo e(route('login')); ?>" class="px-3.5 py-2 bg-[#FF2121]/10 hover:bg-[#FF2121] text-white border border-[#FF2121]/30 hover:border-[#FF2121] rounded-xl text-xs font-bold transition-all duration-300 shadow-sm hover:shadow-[0_0_18px_rgba(255,33,33,0.4)] hover:scale-105 flex items-center gap-1.5 group">
                            <i class="fas fa-sign-in-alt text-xs text-[#FF2121] group-hover:text-white transition-colors"></i>
                            <span>Ingresar</span>
                        </a>

                        <!-- Register / Unirse -->
                        <a href="<?php echo e(route('register')); ?>" class="hidden sm:flex px-4 py-2 gradient-bg rounded-xl text-xs font-black text-white transition hover:scale-105 shadow-lg shadow-[#F51B1B]/20">Unirse</a>
                    <?php endif; ?>
        ...
        <div x-show="mobileMenu" 
            ...
            <div class="flex flex-wrap gap-2">
                <a href="<?php echo e(route('products.index')); ?>" class="px-3 py-1.5 bg-[#FF2121]/10 text-[#FF2121] border border-[#FF2121]/20 text-[10px] font-black uppercase tracking-widest rounded-lg">
                    <i class="fas fa-fire text-[8px] mr-1"></i> Todos
                </a>
                <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="<?php echo e(route('categories.show', $cat->slug)); ?>" class="px-3 py-1.5 bg-white/5 text-gray-400 border border-white/10 text-[10px] font-black uppercase tracking-widest rounded-lg">
                        <i class="<?php echo e($cat->icon ?? 'fas fa-tag'); ?> text-[8px] mr-1"></i> <?php echo e($cat->name); ?>
            ...
            <a href="<?php echo e(route('updates.index')); ?>" class="flex items-center gap-4 text-base font-black uppercase tracking-[0.2em] text-gray-400 active:text-white transition-colors group">
                <i class="fas fa-magic text-amber-500/50 group-active:text-amber-500"></i>
                Actualizaciones
            </a>
            <a href="<?php echo e(route('membership.pricing')); ?>" class="flex items-center gap-4 text-base font-black uppercase tracking-[0.2em] <?php echo e(request()->routeIs('membership.pricing') ? 'text-white' : 'text-gray-400'); ?> active:text-white transition-colors group">
                <i class="fas fa-crown text-yellow-500/50 group-active:text-yellow-500"></i>
                Membresías
            </a>
            <a href="<?php echo e(route('user.support.index')); ?>" class="flex items-center gap-4 text-base font-black uppercase tracking-[0.2em] text-gray-400 active:text-white transition-colors group">
                <i class="fas fa-headset text-[#FF2121]/50 group-active:text-[#FF2121]"></i>
                Soporte
            </a>
            <a href="<?php echo e(route('cart.index')); ?>" class="flex items-center justify-between text-base font-black uppercase tracking-[0.2em] text-gray-400 active:text-white transition-colors group">
            ...
            <?php if(auth()->guard()->check()): ?>
                <a href="<?php echo e(route('dashboard')); ?>" class="flex items-center gap-4 text-base font-black uppercase tracking-[0.2em] text-gray-400 active:text-white transition-colors group">
                    <i class="fas fa-user-circle text-[#FF2121]/50 group-active:text-[#FF2121]"></i>
                    Mi Cuenta
                </a>
            <?php else: ?>
                <div class="grid grid-cols-1 gap-4 pt-2">
                    <a href="<?php echo e(route('login')); ?>" class="w-full py-4 bg-white/5 rounded-2xl text-xs font-black uppercase tracking-widest text-white border border-white/10 text-center">Login</a>
                    <a href="<?php echo e(route('register')); ?>" class="w-full py-4 gradient-bg rounded-2xl text-xs font-black uppercase tracking-widest text-white text-center shadow-lg">Unirse ahora</a>
                </div>
            <?php endif; ?>
        </div>
    </nav>
    ...
    <footer class="relative bg-[#080808] border-t border-white/[0.06] overflow-hidden">
        ...
        <div class="relative max-w-7xl mx-auto px-6 py-16">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div class="relative z-10">
                            <div class="flex items-center gap-4 mb-5">
                                <div class="w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-[#FF2121]/20" style="background: linear-gradient(90deg, #FF2121 0%, #F51B1B 25%, #FF2121 50%, #F51B1B 75%, #FF2121 100%); background-size: 200% auto; animation: footer-logo-shine 6s linear infinite, footer-logo-pulse 4s ease-in-out infinite;">
                                    <i class="<?php echo e($globalSettings['site_icon'] ?? 'fas fa-bolt'); ?> text-lg"></i>
                                </div>
                                <a href="<?php echo e(route('home')); ?>" class="text-2xl font-black tracking-tight text-white"><?php echo e($globalSettings['site_name'] ?? 'WP'); ?> <span class="gradient-text">Market</span></a>
                            </div>
                            <p class="text-gray-400 text-sm leading-relaxed max-w-md">
                                <?php echo e($globalSettings['site_description'] ?? 'La plataforma líder en recursos premium para WordPress. Themes, plugins y herramientas de marketing optimizadas.'); ?>
                ...
                <div class="group relative bg-[#0d0d0d]/80 backdrop-blur-sm border border-white/[0.08] rounded-[32px] p-8 overflow-hidden hover:border-white/20 transition-all duration-500">
                    ...
                    <div class="relative z-10 h-full flex flex-col justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-black text-white"><?php echo e(number_format($productsCount ?? 0)); ?>+</p>
                        </div>
                ...
                <div class="group relative bg-[#0d0d0d]/80 backdrop-blur-sm border border-white/[0.08] rounded-[32px] p-8 overflow-hidden hover:border-white/20 transition-all duration-500">
                    ...
                    <div class="relative z-10">
                        ...
                        <ul class="space-y-3">
                            <li><a href="<?php echo e(route('products.index')); ?>" class="text-sm text-gray-400 hover:text-white transition flex items-center gap-3 group/item"><i class="fas fa-box-open text-xs text-gray-600 group-hover/item:text-[#FF2121] transition"></i> Productos</a></li>
                            <li><a href="<?php echo e(route('pages.rewards')); ?>" class="text-sm text-gray-400 hover:text-white transition flex items-center gap-3 group/item"><i class="fas fa-crown text-xs text-gray-600 group-hover/item:text-[#FF2121] transition"></i> Programa VIP</a></li>
                            <li><a href="<?php echo e(route('pages.plugin')); ?>" class="text-sm text-gray-400 hover:text-white transition flex items-center gap-3 group/item"><i class="fas fa-plug text-xs text-gray-600 group-hover/item:text-[#FF2121] transition"></i> Plugin Oficial</a></li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="group relative bg-[#0d0d0d]/80 backdrop-blur-sm border border-white/[0.08] rounded-[32px] p-8 overflow-hidden hover:border-white/20 transition-all duration-500">
                    ...
                    <div class="relative z-10">
                        ...
                        <ul class="space-y-3">
                            <li><a href="<?php echo e(route('pages.help')); ?>" class="text-sm text-gray-400 hover:text-white transition flex items-center gap-3 group/item"><i class="fas fa-question-circle text-xs text-gray-600 group-hover/item:text-rose-400 transition"></i> Centro de Ayuda</a></li>
                            <li><a href="<?php echo e(route('pages.terms')); ?>" class="text-sm text-gray-400 hover:text-white transition flex items-center gap-3 group/item"><i class="fas fa-file-contract text-xs text-gray-600 group-hover/item:text-rose-400 transition"></i> Términos</a></li>
                            <li><a href="<?php echo e(route('pages.refund')); ?>" class="text-sm text-gray-400 hover:text-white transition flex items-center gap-3 group/item"><i class="fas fa-undo-alt text-xs text-gray-600 group-hover/item:text-rose-400 transition"></i> Reembolso</a></li>
                            <?php if(auth()->guard()->check()): ?>
                                <li><a href="<?php echo e(route('user.support.index')); ?>" class="text-sm text-gray-400 hover:text-white transition flex items-center gap-3 group/item"><i class="fas fa-ticket-alt text-xs text-gray-600 group-hover/item:text-rose-400 transition"></i> Mis Tickets</a></li>
                            <?php endif; ?>
                        </ul>
                    </div>
                </div>
            ...
            <div class="mt-12 pt-8 border-t border-white/[0.06] flex flex-col md:flex-row justify-between items-center gap-4">
                <p class="text-[11px] font-black text-gray-600 uppercase tracking-widest">© <?php echo e(date('Y')); ?> <?php echo e($globalSettings['site_name'] ?? 'WP Marketplace'); ?>. Todos los derechos reservados.</p>
                <div class="flex items-center gap-6 text-[11px] font-black text-gray-600 uppercase tracking-widest">
                    <a href="<?php echo e(route('pages.terms')); ?>" class="hover:text-white transition">Términos</a>
                    <a href="<?php echo e(route('pages.refund')); ?>" class="hover:text-white transition">Reembolso</a>
    ...
    >
        ...
        <div x-show="!loading && searchResults.length > 0" class="p-4 bg-[#050505] border-t border-white/5 shrink-0">
            <a :href="'<?php echo e(route('search.index')); ?>?q=' + encodeURIComponent(searchQuery)" class="w-full py-4 gradient-bg rounded-2xl text-white text-[11px] font-black uppercase tracking-widest text-center block">
                Ver todos los resultados
            </a>
        </div>
    </div>
</body>
</html><?php /**PATH C:\MAMP\htdocs\gplwolf\resources\views/layouts/frontend.blade.php ENDPATH**/ ?>